...
Parse.Cloud.define("searchWorkspaces", function (request, response) {
    ...
    const securedApiKey = "MjA0NDc4N2VhODM5Zjk4YjljMDUyMGVmZTBmYjg1YjgxNzU3NGQ0OTMyNzc1ZmMwYTc5ZGFjZmYwOTA3NTcwZmZpbHRlcnM9X3RhZ3MlM0FZWnpzSXZPVnRzJnVzZXJUb2tlbj1ZWnpzSXZPVnRz"; // Use the key generated earlier

    // only query string
    indexWorkspaces.search('the', function searchDone(err, content) {
...
Parse.Cloud.beforeSave('workspace_follower', function(req, response) {
    ...
    if (!req.user) {
        ...
        if (!req.user.getSessionToken()) {
            ...
            // If isSelected set from client to true, let's update user isSelectedWorkspaceFollower field
            if (workspace_follower.dirty("isSelected")) {

                if (workspace_follower.get("isSelected") === true) {

                    // client set isSelected = true, so let's update user's isSelectedWorkspaceFollower
